...
# Site_ID 4G sites -- new version
rm ${wdir}/4g_cell_status_parsed_enodeb_id.txt
rm ${wdir}/4g_cell_status_parsed_enodeb_id_temp.txt
echo 'SITE;SITE_ID' > ${wdir}/4g_cell_status_parsed_enodeb_id.txt
egrep '1 ENodeBFunction|lget . eNBid' 4g_cell_status.txt > ${wdir}/4g_cell_status_parsed_enodeb_id_temp.txt
perl -ne 'print "$1;" if /(\w+)>/; print "$1\n" if /^.+?ENodeBFunction.+?eNBId\s+(\w+).*$/' ${wdir}/4g_cell_status_parsed_enodeb_id_temp.txt | gsed -r -e 's/^.+?(RN_.+?)(;\w+)$/\1\2 /g' >> ${wdir}/4g_cell_status_parsed_enodeb_id.txt
cp -p ${wdir}/4g_cell_status_parsed_enodeb_id.txt ${wdir}/data/4g_cell_status_parsed_enodeb_id_$(date '+DATE: %d_%m_%Y_%H_%M' | sed 's/DATE: //').txt
